...
export default function TravelInfo() {
  ...
  const [tempTotalPlaces, setTempTotalPlaces] = useState(
    travelData?.totalPlaces || ""
  );
  const [tempTakenPlaces, setTempTakenPlaces] = useState(
    travelData?.takenPlaces || ""
  );
  ...
  const [hours, minutes] = travelData.time
    ? travelData.time.split(":")
    : ["00", "00"];
  ...
  useEffect(() => {
    ...
    editedTravelData.from,
    editedTravelData.to,
    editedTravelData.time,
    editedTravelData.totalPlaces,
    editedTravelData.takenPlaces,
    editedTravelData.carModel,
    auth.currentUser,
    travelData.userId,
  ]);
  ...
  const handleSaveChanges = async () => {
    if (travelData.takenPlaces > tempTotalPlaces) {
      setErrorChanges(
        "ERROR: el número máximo de pasajeros no puede ser superior al numero de plazas reservadas."
      );
      return;
    }
      ...
      if (user) {
        ...
        if (userDocSnapshot.exists()) {
          ...
          if (!reservedTravels.includes(travelData.travelId)) {
            const updatedReservedTravels = [
              ...reservedTravels,
              travelData.travelId,
            ];
            const userDocRef = doc(db, "users", auth.currentUser.uid);
            await updateDoc(userDocRef, {
              reservedTravels: updatedReservedTravels,
            });
            ...
            if (travelDocSnapshot.exists()) {
              ...
              await updateDoc(travelDocRef, {
                carSeatsTaken: updatedTakenPlaces,
              });
      ...
      if (userDocSnapshot.exists()) {
        ...
        if (user.uid === travelData.userId) {
          ...
          if (travelDocSnapshot.exists()) {
            ...
            await updateDoc(travelDocRef, {
              carSeatsTaken: updatedCarSeatsTaken,
            });
            console.log(
              "Reservation canceled successfully. Car seats updated."
            );
          } else {
            console.log("Travel not found.");
          }
  ...
  const confirmCancel = () => {
    Alert.alert("¿Estás seguro de que quieres anular el trayecto?", "", [
            ...
            <View style={styles.contentBottom}>
              <View style={styles.contentInfo}>
                <View style={styles.buttonContainer}>
                  {travelData.buttonText !== "Completo" &&
                  ...
                  {editor ? (
                    ...
                    <TouchableOpacity
                      ...
                      disabled={true}
                    >
                      <Text style={styles.buttonText}>Completo</Text>
                    </TouchableOpacity>
                  )}
                </View>
              </View>
