...
export default function Inicio() {
  ...
  const sortTravelData = (data: any) => {
    return data.sort((a: any, b: any) => {
      const dateA = moment(a.travelDate.toDate()).valueOf();
      const dateB = moment(b.travelDate.toDate()).valueOf();

      if (moment(dateA).isSame(dateB, "day")) {
        const timeA = moment(a.travelTime.toDate()).valueOf();
        const timeB = moment(b.travelTime.toDate()).valueOf();
        return timeA - timeB;
      }
      return dateA - dateB;
    });
                ...
                <TouchableOpacity onPress={() => handleOpenGetPlace()}>
                  ...
                  >
                    <Text style={styles.buttonText}>
                      Empieza a reservar aquí
                    </Text>
                  </LinearGradient>
                </TouchableOpacity>
              </View>
            ) : (
              _.sortBy(travelData, ["travelDate", "travelTime"]).map(
                (travel, index) => {
                  const titleDate = new Date(
                    travel.travelDate.seconds * 1000 +
                      travel.travelDate.nanoseconds / 1e6
                  );
                  const formattedTitle = titleDate.toLocaleDateString("en-GB");

                  const timeDate = new Date(
                    travel.travelTime.seconds * 1000 +
                      travel.travelTime.nanoseconds / 1e6
                  );
                  ...
                  return (
                    <TravelCard
                      ...
                      takenPlaces={
                        travel.carSeatsTaken === ""
                          ? "0"
                          : travel.carSeatsTaken.toString()
                      }
                      iconName="car"
                    />
                  );
